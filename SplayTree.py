...
SplayWrappedDictionary = TypeVar("SplayWrappedDictionary")  # represents a custom type used in application
...
class Node:
    """
    Implementation of an Splay tree node.
    Do not modify.
    """
    ...
    def __repr__(self) -> str:
        """
        Represent the Splay tree node as a string.
...
class SplayTree:
    """
    Implementation of an Splay tree.
    ...
    def __repr__(self) -> str:
        """
        Represent the Splay tree as a string. Inspired by Anna De Biasi (Fall'20 Lead TA).

        :return: string representation of the Splay tree
        """
        ...
        result += "\n"
        result += f"Splay Tree: size = {self.size}, height = {self.root.height}".center(spaces)
        result += "\n\n"
        for i in range(self.root.height + 1):
            result += f"Level {i}: "
    ...
    def height(self, root: Node) -> int:
        """
        Return height of a subtree in the Splay tree, properly handling the case of root = None.
    ...
    def insert(self, root: Node, val: T) -> Node:
        ...
        If `val` already exists in the Splay tree, do nothing.
    ...
    def search(self, root: Node, val: T) -> Node:
        ...
        if root is None:
            ...
            if val < root.left.key:
                root.left.left = self.search(root.left.left, val)
                root = self.right_rotate(root)

            elif val > root.left.key:
                root.left.right = self.search(root.left.right, val)
            ...
            if val > root.right.key:
                root.right.right = self.search(root.right.right, val)
                root = self.left_rotate(root)

            elif val < root.right.key:
                root.right.left = self.search(root.right.left, val)
